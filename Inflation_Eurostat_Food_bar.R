...
dat_2020 <- filter(dat,unit=="RCH_A", coicop =="CP011", geo %in% ct3,time>="2022-04-01") #geo==ct)) & indic_is==ty))#time="2019-01-01")# & indic_is=ty)
...
sel$geo[sel$geo=="CH"] <- "Schweiz"
sel$geo[sel$geo=="DE"] <- "Deutschland"
sel$geo[sel$geo=="FR"] <- "Frankreich"
sel$geo[sel$geo=="AT"] <- "Österreich"
sel$geo[sel$geo=="IT"] <- "Italien"
