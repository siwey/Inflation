...
abstract: 
- Die steigenden Inflationsraten beschäftigen nicht nur Konsumentinnen und Konsumenten, sondern auch die Politik, die   sich überlegen muss, ob und wie sie gegen den Kaufkraftverlust vorgehen will. Mit ihrer überraschend starken         Erhöhung des Leitzinses hat die Schweizerische Nationalbank viele überrascht.

- Es bleiben die preistreibenden Entwicklungen als Folge des Ukrainekriegs und der Corona-Pandemie, wobei diese die    zugrundeliegende Teuerung aufgrund der expansiven Geldpolitik der Notenbanken überlagern. Eine vergleichbare         Situation in Vergangenheit war die «Great depression» in den 70er-Jahren.

- Eine Mehrheit der von NZZ und KOF befragten Ökonomen gehen von einem temporären Anstieg der Inflation aus.

- Der Spielraum für Lohnerhöhungen bleibt überschaubar, denn auch die Unternehmen kämpfen mit beim Bezug von           Vorprodukten mit den höheren Preisen. Dies nagt an der Marge und somit am Spielraum für Lohnerhöhungen. Daneben      kühlt sich auch die wirtschaftliche Entwicklung eher ab, etwa aufgrund der zunehmend restriktiveren Geldpolitik,     des Ukrainie-Kriegs oder der nach wie vor schwierigen Covid-Situation in China.
tags:
- nothing
- nothingness
---
...
**Land** | **Inflationsrate** **`r format(sel_M$Time[length(sel_M$Time)-1],"%b")`** | **Inflationsrate** **`r format(sel_M$Time[length(sel_M$Time)],"%b")`**
------------- | -------------|-------------
**Schweiz** | `r round(sel_M$ObsValue[sel_M$LOCATION=="Schweiz"][length(sel_M$ObsValue[sel_M$LOCATION=="Schweiz"])-1],1)` | `r round(sel_M$ObsValue[sel_M$LOCATION=="Schweiz"][length(sel_M$ObsValue[sel_M$LOCATION=="Schweiz"])],1)`
**Deutschland** | `r round(sel_M$ObsValue[sel_M$LOCATION=="Deutschland"][length(sel_M$ObsValue[sel_M$LOCATION=="Deutschland"])-1],1)` | `r round(sel_M$ObsValue[sel_M$LOCATION=="Deutschland"][length(sel_M$ObsValue[sel_M$LOCATION=="Deutschland"])],1)`
**EU-27** | `r round(sel_M$ObsValue[sel_M$LOCATION=="EU-27"][length(sel_M$ObsValue[sel_M$LOCATION=="EU-27"])-1],1)` | `r round(sel_M$ObsValue[sel_M$LOCATION=="EU-27"][length(sel_M$ObsValue[sel_M$LOCATION=="EU-27"])],1)`
**USA** | `r round(sel_M$ObsValue[sel_M$LOCATION=="USA"][length(sel_M$ObsValue[sel_M$LOCATION=="USA"])-1],1)` | `r round(sel_M$ObsValue[sel_M$LOCATION=="USA"][length(sel_M$ObsValue[sel_M$LOCATION=="USA"])],1)`
...
Als Vorbemerkung vor der Diskussion des Spielraums für Lohnerhöhungen muss angemerkt
...
Branchen Rechnung getragen werden.
...
nominalen Lohnwachstum von 2.6 Prozent aus, wobei dies im laufenden Jahr wegen der
zunehmenden Teuerung nicht zu einer Reallohnerhöhung führen wird. Im kommenden Jahr
sagen die KOF-Prognosen eine durchschnittliche Erhöhung der Nominallöhne um 1.6 und
eine Teuerung von 2.6 Prozent (SNB 2.8 Prozent) voraus, womit die Reallöhne auch 2023
stagnieren dürften.

Ein Blick zurück zeigt, dass die Inflationsraten der Schweiz ab 2009 während mehreren
Jahren negativ waren (vgl. Abbildung \ref{Inf_A}), was sich positiv auf die Entwickung der
Reallöhne auswirkte. So stiegen die Reallöhne in den letzten zehn Jahren um gegen 7 Prozent
...
Betrachtet man die Situation der Unternehmen, so zeigt sich, dass ihnen ebenfalls die hohe
...
auch nicht in ihre Kassen. Im Gegenteil steigen dadurch die Kosten und die Margen sinken.
Nur die wenigsten Unternehmen können die höheren Einkaufspreise durch höhere Preise an
die Konsumenten kompensieren.
...
Konsumverhalten Produkte mit gestiegenen Preisen substituieren. So können Sommerferien
etwa auch in der Schweiz verbracht werden, statt eine teure Flugreise anzutreten.
...
orientiert sich nicht an der Teuerung, viel mehr stellt sich in den Betrieben die Frage, ob nach
Abzug aller Kosten noch Spielraum für Lohnerhöhungen bleibt. Sofern es einen solchen gibt,
so haben auch die Unternehmen einen Anreiz, die Teuerung so gut als möglich auszugleichen,
denn dadurch können sich die Konsumenten mehr leisten.Auch vergeben die Unternehmen die Lohnerhöhungen zunehmend individuell statt generell, um den überdurchschnittlichen Leistungen der Mitarbeitenden Rechnung zu tragen. Ebenso machen sie zunehmend auch vom Mittel der Einmalzahlungen Gebrauch. 

Obwohl die Auftragslage in vielen Betrieben noch gut ist, können sie teilweise wegen der
Lieferkettenprobleme Produkte nicht fertigstellen respektive bleibt ihnen auch beim Verkauf der fertigen Produkte eine tiefere Marge. Eine tiefere Marge senkt in den Betrieben auch den
...
steigenden Krankenkassenprämien oder die hohe Teuerung. Von Letzterer werden die
Betriebe zudem auch selbst negativ tangiert. Die mit Abstand wichtigste Grösse zur
Bestimmung des Spielraums von Lohnerhöhungen ist die aktuelle wirtschaftliche Lage und
die zukünftigen Aussichten, in der sich eine Branche oder ein Betrieb befindet.
...
mit höheren Löhnen kompensieren würden, denn dadurch würde das Signal gesendet, dass es
keinen Handlungsbedarf für nachhaltige Lösungen gebe. Und selbst wenn die Unternehmen
für einen Teil dieser Kosten aufkommen wollten, so fehlen in den meisten Betrieben schlicht
die Mittel dazu.
